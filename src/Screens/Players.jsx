...
export default function Players() {
  ...
  const addPlayer = () => {
    if (name.trim() !== "") {
      setPlayers([...players, { id: Date.now(), name, avatar: getRandomAvatar() }]);
      setName("");
    }
  };
  ...
  return (
    <div className="players-page">
      ...
      <h2 className="title">Ingresa los jugadores</h2>

      <div className="frame-players">
        <img src={frameplayers} alt="Marco de jugadores" className="frame-image" />
        <div className="players-list">
          {players.map((player, index) => (
            <div key={player.id} className="player-item">
              <div className="avatar-container" onClick={(e) => handleAvatarClick(e, index)}>
                <img src={player.avatar} alt={`Avatar de ${player.name}`} className="player-avatar" />
              </div>

              <p className="player-name">{player.name}</p>

              <button onClick={() => removePlayer(index)} className="remove-button">
      ...
      <div className="input-container">
        <img src={playersone} alt="Player sentado" className="playersone-icon"/>
        <input
          type="text"
          value={name}
        ...
        />
        <button onClick={addPlayer} className="add-button">Agregar</button>
      </div>

      <button onClick={() => navigate("/game")} className="start-button" disabled={players.length === 0}>
        Iniciar Juego
      </button>
      ...
      {selectedAvatar !== null && (
        <div className="avatar-options" style={{ top: menuPosition.top, left: menuPosition.left }}>
          ...
          {avatarOptions.map((avatar, i) => (
            <div key={i} className="avatar-option-container" onClick={() => changeAvatar(players[selectedAvatar].id, avatar.src)}>
